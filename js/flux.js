	...
	flux.slider = function(elem, opts) {
		...
		this.element.find('img, a img').each(function(index, found_img){
			...
			if(link.is('a'))
                ...
                $(imgClone).data('data-accessory', link.attr('data-accessory'));
                $(imgClone).data('data-inform', link.attr('data-inform'));
		...
		setupImages: function() {
			var img1 = this.getImage(this.currentImageIndex),
				css1 = {
                    ...
                    'background-size' : window.ELEM_WIDTH,
					'z-index': 101,
					'cursor': 'auto'
				};
			...
			if($(img1).data('href'))
			{
                ...
                this.image1.attr('data-accessory', $(img1).data('data-accessory'));
                this.image1.attr('data-inform', $(img1).data('data-inform'));
		...
		return new flux.transition(fluxslider, $.extend({
			...
			setup: function() {
				...
				this.cubeContainer = $('<div class="cube"></div>').css({
					width: window.ELEM_WIDTH+'px',
					height: window.ELEM_WIDTH+'px',
					position: 'relative'
				}).css3({
					'transition-duration': '800ms',
